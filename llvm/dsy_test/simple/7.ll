; ModuleID = '7.cpp'
source_filename = "7.cpp"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-pc-linux-gnu"

%struct.Point = type { i32, i32, i32 }

@.str = private unnamed_addr constant [2 x i8] c"a\00", align 1
@.str.1 = private unnamed_addr constant [2 x i8] c"b\00", align 1
@.str.2 = private unnamed_addr constant [2 x i8] c"x\00", align 1
@.str.3 = private unnamed_addr constant [2 x i8] c"y\00", align 1
@.str.4 = private unnamed_addr constant [2 x i8] c"c\00", align 1
@.str.5 = private unnamed_addr constant [2 x i8] c"z\00", align 1

; Function Attrs: mustprogress uwtable
define dso_local void @_Z3topv() local_unnamed_addr #0 {
entry:
  %a = alloca i32, align 4
  %b = alloca i32, align 4
  %c = alloca i32, align 4
  %p = alloca %struct.Point, align 4
  %0 = bitcast i32* %a to i8*
  call void @llvm.lifetime.start.p0i8(i64 4, i8* nonnull %0) #3
  %1 = bitcast i32* %b to i8*
  call void @llvm.lifetime.start.p0i8(i64 4, i8* nonnull %1) #3
  %2 = bitcast i32* %c to i8*
  call void @llvm.lifetime.start.p0i8(i64 4, i8* nonnull %2) #3
  %3 = bitcast %struct.Point* %p to i8*
  call void @llvm.lifetime.start.p0i8(i64 12, i8* nonnull %3) #3
  call void @_Z13registerInputPKcPvi(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str, i64 0, i64 0), i8* nonnull %0, i32 4)
  call void @_Z13registerInputPKcPvi(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.1, i64 0, i64 0), i8* nonnull %1, i32 4)
  call void @_Z13registerInputPKcPvi(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.2, i64 0, i64 0), i8* nonnull %3, i32 4)
  %y = getelementptr inbounds %struct.Point, %struct.Point* %p, i64 0, i32 1
  %4 = bitcast i32* %y to i8*
  call void @_Z13registerInputPKcPvi(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.3, i64 0, i64 0), i8* nonnull %4, i32 4)
  %5 = load i32, i32* %a, align 4, !tbaa !3
  %cmp = icmp sgt i32 %5, 10
  br i1 %cmp, label %if.then, label %if.else

if.then:                                          ; preds = %entry
  %x = getelementptr inbounds %struct.Point, %struct.Point* %p, i64 0, i32 0
  %6 = load i32, i32* %x, align 4, !tbaa !7
  %add = add nsw i32 %6, 1
  store i32 %add, i32* %c, align 4, !tbaa !3
  br label %if.end7

if.else:                                          ; preds = %entry
  %cmp2 = icmp slt i32 %5, 2
  br i1 %cmp2, label %if.then3, label %if.else6

if.then3:                                         ; preds = %if.else
  %7 = load i32, i32* %y, align 4, !tbaa !9
  %add5 = add nsw i32 %7, 10
  store i32 %add5, i32* %c, align 4, !tbaa !3
  br label %if.end7

if.else6:                                         ; preds = %if.else
  store i32 5, i32* %c, align 4, !tbaa !3
  br label %if.end7

if.end7:                                          ; preds = %if.then3, %if.else6, %if.then
  %8 = load i32, i32* %b, align 4, !tbaa !3
  %cmp8 = icmp sgt i32 %8, 1
  br i1 %cmp8, label %if.then9, label %if.end11

if.then9:                                         ; preds = %if.end7
  %9 = load i32, i32* %c, align 4, !tbaa !3
  %add10 = add nsw i32 %9, 4
  store i32 %add10, i32* %c, align 4, !tbaa !3
  br label %if.end11

if.end11:                                         ; preds = %if.then9, %if.end7
  %10 = load i32, i32* %c, align 4, !tbaa !3
  %add12 = add nsw i32 %10, 1
  %z = getelementptr inbounds %struct.Point, %struct.Point* %p, i64 0, i32 2
  store i32 %add12, i32* %z, align 4, !tbaa !10
  call void @_Z14registerOutputPKcPvi(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.4, i64 0, i64 0), i8* nonnull %2, i32 4)
  %11 = bitcast i32* %z to i8*
  call void @_Z14registerOutputPKcPvi(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.str.5, i64 0, i64 0), i8* nonnull %11, i32 4)
  call void @llvm.lifetime.end.p0i8(i64 12, i8* nonnull %3) #3
  call void @llvm.lifetime.end.p0i8(i64 4, i8* nonnull %2) #3
  call void @llvm.lifetime.end.p0i8(i64 4, i8* nonnull %1) #3
  call void @llvm.lifetime.end.p0i8(i64 4, i8* nonnull %0) #3
  ret void
}

; Function Attrs: argmemonly mustprogress nofree nosync nounwind willreturn
declare void @llvm.lifetime.start.p0i8(i64 immarg, i8* nocapture) #1

declare dso_local void @_Z13registerInputPKcPvi(i8*, i8*, i32) local_unnamed_addr #2

declare dso_local void @_Z14registerOutputPKcPvi(i8*, i8*, i32) local_unnamed_addr #2

; Function Attrs: argmemonly mustprogress nofree nosync nounwind willreturn
declare void @llvm.lifetime.end.p0i8(i64 immarg, i8* nocapture) #1

attributes #0 = { mustprogress uwtable "frame-pointer"="none" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #1 = { argmemonly mustprogress nofree nosync nounwind willreturn }
attributes #2 = { "frame-pointer"="none" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #3 = { nounwind }

!llvm.module.flags = !{!0, !1}
!llvm.ident = !{!2}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 7, !"uwtable", i32 1}
!2 = !{!"Ubuntu clang version 13.0.1-2ubuntu2.2"}
!3 = !{!4, !4, i64 0}
!4 = !{!"int", !5, i64 0}
!5 = !{!"omnipotent char", !6, i64 0}
!6 = !{!"Simple C++ TBAA"}
!7 = !{!8, !4, i64 0}
!8 = !{!"_ZTS5Point", !4, i64 0, !4, i64 4, !4, i64 8}
!9 = !{!8, !4, i64 4}
!10 = !{!8, !4, i64 8}
